...
export async function deleteOrder(input: { orderId: string }): Promise<ActionResult> {
  ...
  //
  // IMPORTANT : .select() est nécessaire pour DÉTECTER un échec silencieux.
  // Sans .select(), Supabase/PostgREST ne renvoie AUCUNE erreur même si la
  // policy RLS bloque la suppression et que 0 ligne n'a été supprimée —
  // deleteError reste null et on croit à tort que ça a fonctionné.
  ...
  if (!deletedRows || deletedRows.length === 0) {
    console.error('deleteOrder: 0 ligne supprimée (policy RLS DELETE manquante ou commande introuvable)')
    return {
      success: false,
      error: "La suppression a été bloquée par les règles de sécurité de la base de données. Voir la policy RLS 'DELETE' sur la table orders.",
    }
  }
